...
boot_alloc(uint32_t n)
{
 ...
 //    }
    ...
    result = nextfree;
    if ((uint32_t)result > (KERNBASE+npages*PGSIZE)){
        panic("boot_alloc: there is no enough space!\n");
    }
...
mem_init(void)
{
    ...
    // Permissions: kernel RW, user NONE
    // Your code goes here:
    boot_map_region_large(kern_pgdir, (uintptr_t) KERNBASE, ROUNDUP(0xffffffff - KERNBASE, PGSIZE), 0, PTE_W | PTE_P);
...
page_free(struct PageInfo *pp)
{
    ...
    if (pp->pp_ref > 0 || pp->pp_link != NULL) {
        panic("Deallocating page failed");
        return;
    }
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    if (!page_exist){
        ...
        pginfo->pp_ref++;
        *pgdir_entry = page2pa(pginfo) | PTE_U | PTE_W | PTE_P;
    }
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    for (uintptr_t end = va+size; va != end; pa += PGSIZE,va += PGSIZE){
        pte_t *entry = pgdir_walk(pgdir,(const void*)va,1);
        if (entry == NULL){
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
    ...
    pp->pp_ref++;  // adding reference here is crucial
    if (*pgtab & PTE_P) {
